...
#define JUMPOVER_CHAR(p,over) do{for(;*p== over;p++);}whiel(0);
#define JUMPTO_CHAR(p,to) do{for(;*p!=to;p++);}while(0);
//分析获得的请求
int Requset_Parse(struct worker_ctl *wctl)
{
