...
int cgiHandler(struct worker_ctl *wctl)
{
	...
	for(;*pos!='?'&&*pos!='\0';pos++)
		;
	*pos='\0';
	sprintf(rpath,"%s%s","/usr/local/var/www/cgi-bin/",command);
	...
	if(pid<0){
...

		{
			...
			res->status=status;
			send(wctl->conn.cs,res->res.ptr,strlen(res->res.ptr),MSG_CONFIRM);
